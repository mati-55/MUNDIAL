...
#         ttk.Label(header, text="Fase de Grupos - Jornadas", style="Header.TLabel").pack()
...
#         ttk.Label(edit_frame, text="Selecciona una fila y edita los goles:").pack(side='left')
...
#         # collect matches from torneo that correspond to jornada by matching generated_matches list ordering
#         # We'll map pairs by group/jornada across generated_matches
#         idx = 0
#         for mid, p in self.torneo.calendario.items():
#             # Determine group's jornada by comparing pairs in generated_matches
#             # We'll construct mapping from (grupo, equipo1, equipo2) -> jornada
#             pass
#         # Instead of complex mapping by mid, we'll rebuild from generated_matches and map to tournament ids.
#         # Build quick reverse lookup: (id1,id2) -> mid
#         rev = {}
#         for mid,p in self.torneo.calendario.items():
...
#             mid = rev.get((id1,id2))
#             if not mid:
#                 # maybe swapped order in calendario (id2,id1)
#                 mid = rev.get((id2,id1))
...
#             gdisplay = g
#             res = f"{p.goles_e1} : {p.goles_e2}" if p and p.goles_e1 is not None else "PENDIENTE"
#             self.tree.insert("", tk.END, iid=mid, values=(mid, gdisplay, e1, p.goles_e1 if p.goles_e1 is not None else "", "vs", p.goles_e2 if p.goles_e2 is not None else "", e2, res))
...
#             # update team stats via registrar_resultado (but we must avoid double counting if saved multiple times)
#             # Simple approach: call registrar_resultado only first time; if overwritten later, we won't recompute historical - acceptable for this implementation
#             # We'll force re-calc: naive approach is ok for small app: just set and then recompute all stats from scratch
#             self._recompute_stats_from_results()
#             self.torneo.guardar_datos()
#             win.destroy()
#             self._load_jornada(self.current_jornada)
#         ttk.Button(win, text="Guardar", command=save).pack(pady=6)
...
#         for mid,p in self.torneo.calendario.items():
#             if p.goles_e1 is None: continue
...
#             mid = vals[0]
#             # values positions: (ID,Grupo,Equipo1,G1,"vs",G2,Equipo2,Resultado)
#             try:
#                 g1 = vals[3]
#                 g2 = vals[5]
#                 # allow empty -> skip
